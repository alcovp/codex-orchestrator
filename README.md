...
- Вызовы Codex CLI работают через tee: stdout/stderr Codex транслируются в stdout/stderr оркестратора и попадают в общий лог запуска. В памяти хранится только хвост вывода (по умолчанию 2MB) для парсинга JSON. Чтобы отключить транслирование, задайте `ORCHESTRATOR_TEE_CODEX=0`.
