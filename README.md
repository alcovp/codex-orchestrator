...
## Хранилище состояния
- В корне запуска оркестратора создаётся `orchestrator-db.json` (lowdb). В нём хранятся джобы, их статус, подзадачи (запланирована/в работе/готова/failed), результаты воркеров, а также JSON-артефакты, которыми обмениваются Codex-прогоны (план, summaries подзадач, merge-результат и вход merge).
- Флаги окружения:
