...
## Хранилище состояния
- В корне запуска оркестратора (не целевого репо) создаётся `orchestrator-db.json` (lowdb). Путь можно переопределить переменной `ORCHESTRATOR_DB_PATH`. В БД хранятся джобы, их статус, подзадачи (запланирована/в работе/готова/failed), результаты воркеров, а также JSON-артефакты, которыми обмениваются Codex-прогоны (план, summaries подзадач, merge-результат и вход merge).
- Флаги окружения:
